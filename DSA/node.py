...
#Here I am creating the Nodes
yacko = Node("likes to yak")
wacko = Node("has a penchant for hoarding snacks")
dot = Node("enjoys spending time in movie lots")

#Here I am creating the link nodes. So I am setting dot to be the next node of yacko and wacko to be the next node of dot.
yacko.set_link_node(dot)
dot.set_link_node(wacko)

#Here I am returning the value of the next node of yacko and dot.
# yacko.get_link_node() -> returns the next node of yacko (which is dot) and get_value() returns the value of that node (dot). >>>same with wackos_data.
